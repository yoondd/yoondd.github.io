...
function solution(friends, gifts) {
    ...
    const before = [];
    const giftInfos = gifts.map(item => item.split(" "));
    const friendInfos = [];
    const after = [];
    ...
    for( let item of giftInfos ){
        const sendidx = friends.indexOf(item[0]);
        const getidx = friends.indexOf(item[1]);
        before[sendidx][getidx]++;
    }

    friends.forEach( (item,idx) => {
        let myPoint = 0;
        for( let i = 0; i < friends.length; i++ ){
            myPoint += before[idx][i] - before[i][idx];
        }
        const friendinfo = {
            name: item,
            point: myPoint,
        ...
        for( let j = 0; j < friends.length; j++){
            ...
            if (i > j){
                if( before[i][j] == before[j][i] ) {
                    if( friendInfos[i].point == friendInfos[j].point ) {
                        receiver = null;
                    }else{
                        receiver = friendInfos[i].point > friendInfos[j].point ? friends[i]: friends[j];
                    }
                    
                }else{
                    receiver = before[i][j] > before[j][i] ? friends[i] : friends[j];
                }
            }
            inner[j] = receiver;
            for( let item of friendInfos ){
                if( item.name == receiver ) item.will++;
            }
        }
        after[i] = inner;
    }
    ...
    for( let item of friendInfos ){
        answer = item.will >= answer ? item.will : answer;
    }
